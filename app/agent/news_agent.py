...
""" + ",\n".join([f"""                    {{
    ...
    async def analyze_and_save_news(
        ...
        # === PASO 1: Crear batches y lanzar análisis EN PARALELO ===
        ...
        # === PASO 2: Esperar todos los análisis (máximo en paralelo = semaphore) ===
        ...
        # === PASO 3: Pre-preparar entidades para BULK INSERT ===
        ...
        # Cargar URLs existentes (una sola query)
        start = time.perf_counter()
        ...
        news_db_map = {}  # url → news_id para referencias posteriores
        ...
        for batch_news, analyses in batch_results:
            for news_item, analysis_dict in zip(batch_news, analyses):
                try:
                    ...
                    # Preparar noticia para inserción
        ...
        # === PASO 4: BULK INSERT (reducir a pocas queries) ===
        ...
        # Obtener noticias (para vincular después)
        start = time.perf_counter()
        ...
        # Insertar análisis personalizados
        start = time.perf_counter()
        analyses_bulk = []
        for batch_news, analyses in batch_results:
            for news_item, analysis_dict in zip(batch_news, analyses):
